...
pub fn all() -> Vec<Profile> {
    let mut profiles = vec![cachy(), fedora(), ubuntu(), dev()];
...
pub fn find(name: &str) -> Option<Profile> {
    ...
    for p in [cachy(), fedora(), ubuntu(), dev()] {
        if p.name == lower {
            return Some(p);
        }
    }
...
fn ubuntu() -> Profile {
    Profile {
        name: "ubuntu".into(),
        label: "Ubuntu".into(),
        description: "Ubuntu-based general-purpose environment".into(),
        toml: include_str!("profiles/ubuntu.toml").into(),
    }
}
...
fn dev() -> Profile {
    Profile {
        name: "dev".into(),
        label: "Dev".into(),
        description: "CachyOS-based development environment with mise, neovim, git, and dev tooling pre-installed".into(),
        toml: include_str!("profiles/dev.toml").into(),
    }
}
