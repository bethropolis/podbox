...
pub fn resolve_extends_chain(initial_path: &Path, initial_toml: &str) -> Result<toml::Value> {
    ...
    let mut current_val: toml::Value = toml::from_str(initial_toml)
        .with_context(|| format!("failed to parse TOML at '{}'", initial_path.display()))?;
    ...
    let canon_initial =
        std::fs::canonicalize(initial_path).unwrap_or_else(|_| initial_path.to_path_buf());
    ...
    while let Some(extends_val) = current_val.get("extends").and_then(|v| v.as_str()) {
        ...
        let (source, next_raw, next_dir) = resolve_extends_target(trimmed, &current_dir)
            .with_context(|| {
                format!(
        ...
        current_val = toml::from_str(&next_raw)
            .with_context(|| format!("failed to parse TOML for extends target {source:?}"))?;
...
fn resolve_extends_target(
    ...
    if is_path_like {
        ...
        let content = std::fs::read_to_string(&candidate)
            .with_context(|| format!("failed to read extends path '{}'", candidate.display()))?;
