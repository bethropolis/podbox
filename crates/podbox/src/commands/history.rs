...
pub fn run_history(
    ...
    for e in filtered {
        let action = match e.action.as_str() {
            "start" | "enable" | "recover" => {
                e.action.if_supports_color(Stream::Stdout, |s| s.green()).to_string()
            }
            "stop" | "disable" | "remove" => {
                e.action.if_supports_color(Stream::Stdout, |s| s.red()).to_string()
            }
            _ => e.action.if_supports_color(Stream::Stdout, |s| s.cyan()).to_string(),
        };
        println!(
            "{:<20} {:<22} {:<14} {}",
