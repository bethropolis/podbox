...
/// Run clipboard operations: `podmgr-clipboard set` or `podmgr-clipboard get`.
pub fn run(args: &[String]) {
    if args.len() < 2 {
        eprintln!("podmgr-clipboard: expected 'set' or 'get'");
        std::process::exit(1);
    }
    ...
    match args[1].as_str() {
        ...
        _ => {
            eprintln!("podmgr-clipboard: unknown command '{}'", args[1]);
            std::process::exit(1);
        }
    }
}
