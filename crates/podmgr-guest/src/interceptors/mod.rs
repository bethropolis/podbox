...
fn host_socket_path() -> Result<PathBuf, crate::error::GuestError> {
    ...
    Ok(PathBuf::from(&xdg_runtime).join("podmgr").join(format!("{}.sock", container_name)))
}
...
fn send_to_host_and_read_response(msg: &GuestMessage) -> Result<HostMessage, crate::error::GuestError> {
    let path = host_socket_path()?;
    let mut stream = UnixStream::connect(&path)?;
    write_frame(&mut stream, msg)?;
    let response = read_frame(&mut stream)?
        .ok_or_else(|| crate::error::GuestError::HandshakeFailed("host closed connection".into()))?;
