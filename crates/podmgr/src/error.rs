...
pub enum PodmgrError {
    #[error("container '{0}' not found -- run `podmgr build` and `podmgr enable` first")]
    ...
    #[error("podmgr-guest binary not found -- build it with: cargo build -p podmgr-guest --release --target x86_64-unknown-linux-musl")]
    GuestBinaryNotFound,
